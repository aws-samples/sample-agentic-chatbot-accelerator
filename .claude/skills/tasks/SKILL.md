...
1. **Read** the design doc, especially its Task map table (# / Task / Requirements / Depends on / File), its Contract to preserve, and its Edge cases & failure handling table. If there is no task map, STOP and recommend `/design-doc <slug>` first.
...
4. **Generate one file per task**, `tasks/T<N>-<slug>.md`, each with: a metadata table (Status / Depends-on / Requirements) followed by a **Satisfies** bullet list — one verbatim-quoted story DoD checkbox per bullet, plus the design doc's `FR`/contract ids this task discharges — then Objective, Files, an **API surface** for that unit, Notes/gotchas, any Edge cases owned, an Acceptance checkbox list, and any task-local Decisions.
   - **Carry the design doc's edge-case rows into the owning task, verbatim.** A failure-handling table that no task file references is a table the building phase never reads. Each row belongs to exactly one task; if a row has no owner, say so in the index rather than dropping it.
...
5. **Keep task files consistent with the design doc** — same file paths, same dependency edges, same DoD mapping. Every DoD checkbox, every `FR`, every contract row, and every edge-case row is either discharged by a task's Acceptance or explicitly listed as accepted; flag any gap rather than papering over it.
